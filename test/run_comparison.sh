...
run_ttbar() {
   ...
   cmsDriver.py step2  --conditions auto:$conditions -s DIGI:pdigi_valid,L1,DIGI2RAW,HLT:@fake --datatier GEN-SIM-DIGI-RAW -n $events $geometry --era $era --eventcontent FEVTDEBUGHLT --fileout=step2.root --filein=step1.root &> step2.log
   cmsDriver.py step3  --conditions auto:$conditions -n $events --era $era --eventcontent RECOSIM,MINIAODSIM,DQM --runUnscheduled  -s RAW2DIGI,L1Reco,RECO,EI,PAT,VALIDATION:@standardValidation+@miniAODValidation,DQM:@standardDQM+@miniAODDQM --datatier GEN-SIM-RECO,MINIAODSIM,DQMIO --fileout=step3.root --filein=step2.root $geometry &> step3.log
}
